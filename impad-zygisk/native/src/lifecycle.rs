...
pub unsafe fn do_pre_app_specialize(module: &mut ImpPadModule, args: *mut AppSpecializeArgs) {
    ...
    // impad: 不查 companion —— 本模块对所有可注入进程全量注入。
    // 真正的「目标是否启用」由 Kotlin 侧 ZygiskEntry.init 根据 11 宿主
    // 白名单决定（只在匹配包内继续安装 hook）。
...
pub unsafe fn do_post_app_specialize(module: &mut ImpPadModule, _args: *const AppSpecializeArgs) {
    if !module.enabled {
        return;
    }
    ...
    // Copy APK
    let apk_dst = format!("{data_dir}/files/mmkv/.impad-bootstrap.apk");
    if !crate::payload::copy_module_file(
        ...
        loge!("Zygisk: failed to copy impad.apk");
        return;
    }
    ...
    for name in module.dex_names.clone() {
        let dst = format!("{data_dir}/files/mmkv/.impad-bootstrap-{name}");
        if !crate::payload::copy_module_file(
            mod_fd,
            &format!("payload/{name}"),
            &dst,
    ...
    let entry_name = "com.impad.pro.zygisk.ZygiskEntry";
...
pub unsafe fn do_pre_server_specialize(module: &mut ImpPadModule, _args: *mut ServerSpecializeArgs) {
    (*module.api).set_option(DLCLOSE_MODULE_LIBRARY);
}
